...
def setup(conf, base_url=None, skip_ws=False):
    # config_path = os.path.join(os.path.dirname(__file__), "config.json")
    # with open(config_path) as f:
        # config = json.load(f)
    # with open('/backend-api/bots/credentials/master_account/connectors/hyperliquid_perpetual.yml',):
    account: LocalAccount = eth_account.Account.from_key(conf["hyperliquid_perpetual_api_secret"])
    address = conf["hyperliquid_perpetual_api_key"]
    if address == "":
        address = account.address
    logging.info(f"Running with account address: {address} {conf}")
    if address != account.address:
        logging.info(f"Running with agent address: {account.address}")
